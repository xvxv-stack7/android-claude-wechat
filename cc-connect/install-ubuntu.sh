...
# 在 proot-distro login ubuntu --isolated 里跑
# ============================================
set -e
...
echo ""
echo "===== 第1步：检查网络 ====="
ping -c 1 -W 3 registry.npmmirror.com > /dev/null 2>&1 && echo "[ok] 网络通" || { echo "[note] 网络不通但不影响测试，继续..."; }

echo ""
echo "===== 第2步：装依赖 ====="
apt update -y && apt install nodejs npm curl -y
echo "[ok] 依赖安装完成"
...
curl -L --connect-timeout 5 --max-time 60 -o /tmp/${CC_FILE} "https://github.com/chenhg5/cc-connect/releases/download/${CC_VERSION}/${CC_FILE}" || curl -L --max-time 60 -o /tmp/${CC_FILE} "https://ghproxy.net/https://github.com/chenhg5/cc-connect/releases/download/${CC_VERSION}/${CC_FILE}"

NODE_DIR=$(npm root -g)/cc-connect
mkdir -p ${NODE_DIR}/bin
tar xzf /tmp/${CC_FILE} -C /tmp/
cp /tmp/cc-connect ${NODE_DIR}/bin/cc-connect
...
#!/bin/bash
export HOME=/root
exec /usr/local/lib/node_modules/cc-connect/bin/cc-connect \
